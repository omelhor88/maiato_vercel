...
interface CustomerInsuranceProps {
  customerId: string;
  customerName?: string;
}

export const CustomerInsurance = ({ customerId, customerName }: CustomerInsuranceProps) => {
  ...
  // Auto-fill NIF when customer is selected
  ...
  const { data: insurance } = useQuery({
    queryKey: ["customer-insurance", customerId, customerNifNormalized],
    ...
    queryFn: async () => {
      ...
      // Fetch receipts for each insurance
      const insuranceWithReceipts = await Promise.all(
        (data || []).map(async (ins) => {
          if (!ins.numero_apolice) return { ...ins, receipt: null };
  ...
  const createInsuranceMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const inferredNif = customerNifNormalized || normalizeNif(data.nif) || data.nif;
      if (inferredNif) insertData.nif = inferredNif;
      ...
      const { data: insuranceData, error } = await supabase.from("insurance").insert([insertData]).select().single();
      if (error) throw error;
      
      // Create insurance receipt
      const receiptData: any = {
        apolice_numero: insuranceData.numero_apolice,
        user_id: user?.id || '00000000-0000-0000-0000-000000000000',
        customer_id: data.customer_id,
      ...
      const { error: receiptError } = await supabase.from("insurance_receipts").insert([receiptData]);
  ...
  const updateInsuranceMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      ...
      // Update or create insurance receipt
      const { data: existingReceipt } = await supabase
        .from("insurance_receipts")
        .select()
        .eq("apolice_numero", insuranceData.numero_apolice)
        .maybeSingle();
      ...
      if (existingReceipt) {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        const { error: receiptError } = await supabase.from("insurance_receipts").insert([{
          ...receiptData,
          apolice_numero: insuranceData.numero_apolice,
          user_id: user?.id || '00000000-0000-0000-0000-000000000000',
          customer_id: data.customer_id,
  ...
  const handleEditInsurance = (ins: any) => {
    ...
    setInsuranceFormData({
      ...
      premio_total: ins.receipt?.premio_total ? Math.abs(ins.receipt.premio_total).toString() : "",
      recebido_maiato: ins.receipt?.recebido_maiato || false,
      pago_companhia: ins.receipt?.pago_companhia || false,
      estorno: ins.receipt?.estorno || false,
      data_pagamento: ins.receipt?.data_pagamento || "",
    });
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={insuranceFormData.premio_total}
                          onChange={(e) => setInsuranceFormData({ ...insuranceFormData, premio_total: e.target.value })}
                          placeholder="0.00"
                        />
                      </div>
                      ...
                      <div className="space-y-2 flex items-center gap-2">
                        <Checkbox
                          id="estorno"
                          checked={insuranceFormData.estorno}
                          onCheckedChange={(checked) => 
                            setInsuranceFormData({ ...insuranceFormData, estorno: checked === true })
                          }
                        />
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          type="date"
                          value={insuranceFormData.data_pagamento}
                          onChange={(e) => setInsuranceFormData({ ...insuranceFormData, data_pagamento: e.target.value })}
                        />
                      </div>
                      ...
                      <div className="space-y-2 flex items-center gap-2">
                        <Checkbox 
                          id="recebido_maiato_customer"
                          checked={insuranceFormData.recebido_maiato} 
                          onCheckedChange={(checked) => {
                            setInsuranceFormData({ 
                              ...insuranceFormData, 
                        ...
                        />
                        <Label htmlFor="recebido_maiato_customer" className="cursor-pointer">Recebido Maiato</Label>
                      </div>

                      {/* Pago Companhia */}
                      <div className="space-y-2 flex items-center gap-2">
                        <Checkbox 
                          id="pago_companhia_customer"
                          checked={insuranceFormData.pago_companhia} 
                          onCheckedChange={(checked) => {
                            setInsuranceFormData({ 
                              ...insuranceFormData, 
                        ...
                        />
                        <Label htmlFor="pago_companhia_customer" className="cursor-pointer">Pago Companhia</Label>
                      </div>
                    </div>
            ...
            {selectedInsurance && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedInsurance.receipt?.premio_total && (
                    <div>
                      <Label>Prémio Total</Label>
                      <p className="text-sm">€{selectedInsurance.receipt.premio_total}</p>
                    </div>
                  )}
                  {selectedInsurance.receipt?.data_pagamento && (
                    <div>
                      <Label>Data Pagamento</Label>
                      <p className="text-sm">{new Date(selectedInsurance.receipt.data_pagamento).toLocaleDateString()}</p>
                    </div>
                  )}
                  {selectedInsurance.receipt?.estorno && (
                    <div>
                      <Label>Estorno</Label>
